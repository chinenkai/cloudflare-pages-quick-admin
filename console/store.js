...
import f404 from './template/404.vue'

import form from './template/formPage.vue'
import table from './template/tablePage.vue'
import card from './template/cardPage.vue'
...
var funs = {
    ...
    initVue: function(router, $CONFIG) {
        window.VueApp = Vue.createApp(app);
        // window.VueApp.config.globalProperties.$router = router;
        window.VueApp.config.globalProperties.$state = state;
        window.VueApp.config.globalProperties.$funs = funs;
        window.VueApp.config.globalProperties.$CONFIG = $CONFIG;
        window.VueApp.use(router);
        window.VueApp.use(TDesign);
            ...
            for (var j = 0; j < topMenu.children.length; j++) {
                ...
                for (var k = 0; k < subMenu.children.length; k++) {
                    ...
                    if (!terMenu.children || terMenu.children.length <= 0) {
                        ...
                        var temp_path = terMenu.path + ':pathMatch(.*)*';
                    ...
                    for (var l = 0; l < terMenu.children.length; l++) {
                        ...
                        var temp_path = fouMenu.path + ':pathMatch(.*)*';
